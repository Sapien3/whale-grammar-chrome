...
const EVENT_LISTENER = {
  ...
  _isEditableNode: function (nodeEl) {
    if (nodeEl.nodeName === `TEXTAREA` || nodeEl.contentEditable === `${true}`) {
      return true;
    }
  ...
  _isNeedReset: function (nodeEl) {
    if (this.lastActiveEl && this._isEditableNode(nodeEl) && nodeEl !== this.lastActiveEl)
      return true;
  ...
  initializeListener: function () {
    ...
    this.port.onMessage.addListener((message) => {
      ...
      if (PORT_LISTENER.hasOwnProperty(action) && typeof PORT_LISTENER[action] === `function`) {
  ...
  onDocumentFocused: function () {
    console.log("onDocumentFocused lol");

    const { activeElement } = document;
    if (this._isEditableNode(activeElement) === false || activeElement.offsetHeight < 50) {
      return;
    }
    ...
    if (activeElement.nodeName === `TEXTAREA`) {
      ...
      const { styleObserver, inputEventListener, scrollEventListener } =
        onEditableElementFocused(options);
      this.styleObserver = styleObserver;
      this.inputListener = inputEventListener;
      this.scrollListener = scrollEventListener;
    }
